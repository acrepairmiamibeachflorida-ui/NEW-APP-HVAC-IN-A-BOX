      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-center justify-center bg-black/75 p-4">
        <motion.div initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.96, opacity: 0 }} className="w-full max-w-md rounded-[24px] border border-white/10 bg-slate-950 p-6 text-white shadow-2xl">
          ...
          <div className="mt-6 flex flex-wrap justify-end gap-3">
            {actions.map((action) => (
              <button key={action.label} onClick={action.onClick} className={`rounded-xl px-4 py-3 text-sm font-semibold ${action.primary ? "bg-white text-slate-950" : "bg-white/10 text-white"}`} style={{ border: "none" }}>
                {action.label}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-white/10 bg-black/85 p-4 backdrop-blur-xl">
        <button onClick={() => setOpen(true)} className="text-xl text-white" style={{ border: "none", background: "transparent" }}>☰</button>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={saveAndExit} className="rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-white" style={{ border: "none" }}>Save & Exit</button>
        </div>
      </header>
          ...
          <motion.div initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} className="fixed inset-y-0 left-0 z-50 w-64 border-r border-white/10 bg-black p-5">
            <div className="mb-6">
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <button onClick={() => setOpen(false)} style={{ border: "none", background: "transparent", color: "white" }}>✕</button>
              </div>
            </div>
                      ...
                      if (resultsLocked) {
                        setModal({
                          ...
                          message: !setupComplete
                            ? "This page is only available after Business Setup is fully completed and all 12 questions are answered to ensure the most accurate analysis."
                            : "This page is only available after all 12 questions are completed to ensure the most accurate analysis.",
                          kind: "warning",
                          actions: [
                            { label: "Continue Diagnostic", onClick: () => { setModal(null); setView(setupComplete ? "workbook" : "profile"); setOpen(false); }, primary: true },
    ...
    content = (
      <div className="space-y-4">
        ...
        {[...currentStep.checklist, NONE].map((item, idx) => (
          <div key={`${item}-${idx}`} onClick={() => setChecklistValue(idx, !currentResponse.checklist[idx])} className={`rounded-xl border p-5 ${currentResponse.checklist[idx] ? "border-amber-300 bg-amber-300/10" : "border-white/10 bg-white/5"}`}>
            {item}
          </div>
        ))}
